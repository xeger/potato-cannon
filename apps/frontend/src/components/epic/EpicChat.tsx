...
import { Send, Loader2, Bot, Brain, AlertCircle } from 'lucide-react'
...
import { useSessionOutput, useSessionEnded, useEpicMessage } from '@/hooks/useSSE'
...
export function EpicChat({ projectId, epicId }: EpicChatProps) {
  ...
  useSessionOutput(useCallback((data: { epicId?: string; event?: { type?: string; message?: { content?: Array<{ type?: string; name?: string; input?: Record<string, unknown>; text?: string }> } } }) => {
    // Only process events for this epic
    if (data.epicId !== epicId) return
  ...
  // Subscribe to epic messages (notifications, user messages, and questions) via SSE
  useEpicMessage(useCallback((data: { epicId?: string; message?: { type?: string; text?: string; timestamp?: string; conversationId?: string; options?: string[] } }) => {
    // Only process messages for this epic
    if (data.epicId !== epicId) return
  ...
  useSessionEnded(useCallback((data: { epicId?: string; exitCode?: number; status?: string }) => {
    if (data.epicId !== epicId) return
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        ...
        console.error('Failed to load epic message history:', error)
      } finally {
        if (!cancelled) {
  ...
  const handleSend = useCallback(async (text: string) => {
    ...
    try {
      await api.sendEpicChatMessage(projectId, epicId, messageText)
        ...
        <div className="space-y-4 py-4">
          ...
          {!isLoadingHistory && messages.length === 0 && (
            <div className="text-center py-8 text-text-muted text-sm">
              Start a conversation about this epic.
            </div>
          )}

          {messages.map((message, index) => (
            <EpicMessageBubble key={index} message={message} />
          ))}
...
interface EpicMessageBubbleProps {
  message: BrainstormMessage
}

function EpicMessageBubble({ message }: EpicMessageBubbleProps) {
